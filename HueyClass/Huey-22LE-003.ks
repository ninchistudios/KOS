...
print " ".
print "################################".
print "# HUEY-22LE-003 FLIGHT PROGRAM #".
print "# RELAYSAT I POLAR INSERTION   #".
print "################################".
print " ".
...
local TGT_APO is 250000. // target orbit to be circularised
local TGT_PERI is 220000. // target orbit to be circularised
local TGT_ASC_ROLL is 270. // do a special ascent roll program stage
local TGT_INCL is 180. // target orbital inclination, Canaveral lowest energy launch = 118.5, polar is 0/180
...
local BLOWN_APO_CUTOFF is TGT_APO * 1.1.
lock BLOWN_APO to (MY_VESSEL:APOAPSIS > BLOWN_APO_CUTOFF).
...
function doSetup {
  ...
  print "Target Orbit: A: " + TGT_APO/1000 + "km / P: " + TGT_PERI/1000 + "km".
  print "Target Orbital Inclination: " + TGT_INCL + " deg".
        ...
        when simpleStageNeeded(MY_VESSEL) then {
          ...
          when simpleStageNeeded(MY_VESSEL) then {
            ...
            when simpleStageNeeded(MY_VESSEL) then {
              ...
              when (MY_VESSEL:PERIAPSIS > TGT_PERI OR BLOWN_APO) then {
                lock throttle to 0.
                if (BLOWN_APO) {
                  print "# APOAPSIS ABORT - INSERTION STAGE SHUTDOWN AT T+" + ROUND((TIME:SECONDS - START_TIME),0).
                } else {
