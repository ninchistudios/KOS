...
- Launching from Woomera (~31deg S) or Cape York (~12deg S), not KSC.
- preserve exisiting comments and comment blocks in scripts for documentation and future standardisation
- all flight config variables go in the `CONFIGURE FLIGHT` block at the top of the mission script
...
- /boot/[mission].ks - Boot file loaded in the vehicle. Multi stage or launcher return missions may require multiples.
- /common/Utils.ks - Common small utils used by the mission script or flight phase utils. Loaded first in the bootfile.
- /common/[Phase]Utils.ks - Utilities for each phase of flight. Loaded after Utils.
- /mission/[mission].ks - The mission specifics, utilising common utils as needed. 
